...
class LogisticParameterManager:
    ...
    def __init__(self, vary_time=False, vary_skypos=False, **kwargs):
        ...
        # add eccentricity and radial anomaly, if we want to sample them;
        # otherwise fix them to zero. NB: `mean_per_ano` is passed to gwsignal
        # as meanPerAno; for the pyseobnr eccentric models its default
        # interpretation is the relativistic anomaly, unless
        # {"radial_anomaly_type": "mean_anomaly"} is passed via
        # waveform_kwargs.
        self.vary_eccentricity = kwargs.get('vary_eccentricity', False)
        if self.vary_eccentricity:
            if not self.use_gwsignal:
                raise ValueError(
                    "--vary-eccentricity requires a gwsignal approximant "
                    f"(e.g. SEOBNRv5EHM); {self.approx_name} uses the lal "
                    "interface, which ignores eccentricity")
            self.logistic_parameters.append(
                LogisticParameter('eccentricity', kwargs.get('ecc_lim', [0., 0.5]), None)
            )
            self.cartesian_angles.append(
                CartesianAngle('mean_per_ano', phase_offset=np.pi)
            )
        else:
            self.fixed['eccentricity'] = 0.
            self.fixed['mean_per_ano'] = 0.
    ...
    def initialize_walkers(self, nwalkers, injected_parameters, reference_posterior=None, verbose=False):
        ...
        if reference_posterior is None:
            ...
            for param in logistic_params:
                ...
                # draw walkers; if the reference posterior does not contain
                # this parameter (e.g. eccentricity), keep the random
                # initialization from above
                try:
                    walkers_phys = np.asarray(reference_posterior[param.physical_name])[idxs]
                except KeyError:
                    print(f"{param.physical_name} not in reference posterior, "
            ...
            for param in cartesian_params:
                ...
                try:
                    ...
                    print(f"{param_kw} not in reference posterior, "
                          "initializing walkers randomly")
                    continue
